...
! that use 1 for their integration.
...
contains
  ...
  subroutine vode_integrator(state_in, state_out, dt, time, status)
    ...
    ! We want 1 to re-initialize each time we call it.
    ...
    ! Set the maximum number of steps allowed (the 1 default is 500).
    ...
    ! Convert the EOS state data into the form 1 expects.
    ...
    ! 1 does not always fail even though it can lead to unphysical states,
    ! so add some sanity checks that trigger a retry even if 1 thinks
    ...
    ! get the number of RHS calls and jac evaluations from the 1
    ! work arrays
