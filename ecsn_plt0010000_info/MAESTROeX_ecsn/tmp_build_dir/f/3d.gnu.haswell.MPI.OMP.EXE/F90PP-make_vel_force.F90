...
contains
  ...
  subroutine make_vel_force(lo, hi, &
    ...
    double precision, intent (inout) :: vel_force(f_lo(1):f_hi(1),f_lo(2):f_hi(2),f_lo(3):f_hi(3),3)
    double precision, intent (in   ) ::       gpi(g_lo(1):g_hi(1),g_lo(2):g_hi(2),g_lo(3):g_hi(3),3)
    ...
    double precision, intent (in   ) :: w0_force_cart(wf_lo(1):wf_hi(1),wf_lo(2):wf_hi(2),wf_lo(3):wf_hi(3),3)
    double precision, intent (in   ) ::       rho0_cart(r0_lo(1):r0_hi(1),r0_lo(2):r0_hi(2),r0_lo(3):r0_hi(3))
    double precision, intent (in   ) :: grav_cart(gr_lo(1):gr_hi(1),gr_lo(2):gr_hi(2),gr_lo(3):gr_hi(3),3)
    ...
    vel_force(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),1:3) = ZERO
    ...
    do k = lo(3),hi(3)
       do j = lo(2),hi(2)
          do i = lo(1),hi(1)
             ...
             vel_force(i,j,k,1:3-1) = -gpi(i,j,k,1:3-1) / rho(i,j,k)

             vel_force(i,j,k,3) = &
                (rhopert * grav_cart(i,j,k,3) - gpi(i,j,k,3)) / rho(i,j,k) - w0_force_cart(i,j,k,3)
          ...
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                ...
                if (r .le. -1) then
                    ...
                    else if (r .gt. domhi(3)) then
                    ! do not modify force since dw0/dr=0
                    else
                    vel_force(i,j,k,3) = vel_force(i,j,k,3) &
                            - (wedge(i,j,k+1)+wedge(i,j,k))*(w0_cart(i,j,k+1,3)-w0_cart(i,j,k,3)) / (2.d0*dx(3))
                endif
