...
module extern_probin_module
  ...
  logical, save, public :: disable_thermal_neutrinos = .false.
  !$acc declare create(disable_thermal_neutrinos)
  ...
  integer, save, public :: nonaka_i = 0
  !$acc declare create(nonaka_i)
  integer, save, public :: nonaka_j = 0
  !$acc declare create(nonaka_j)
  integer, save, public :: nonaka_k = 0
  !$acc declare create(nonaka_k)
  character (len=256), save, public :: nonaka_file = "nonaka_plot.dat"
  !$acc declare create(nonaka_file)
...
module runtime_init_module
  ...
  namelist /probin/ disable_thermal_neutrinos
  ...
  namelist /probin/ use_jacobian_caching
  namelist /probin/ nonaka_i
  namelist /probin/ nonaka_j
  namelist /probin/ nonaka_k
  namelist /probin/ nonaka_file
...
subroutine runtime_init()
  ...
  eos_dtol = 1.0d-8
  small_x = 1.d-30
  disable_thermal_neutrinos = .false.
  use_tables = .false.
  ...
  !$acc device(eos_dtol, small_x, disable_thermal_neutrinos) &
  !$acc device(use_tables, use_c12ag_deboer17, scaling_method) &
  ...
  !$acc device(reactions_energy_scale, ode_max_steps, use_jacobian_caching) &
  !$acc device(nonaka_i, nonaka_j, nonaka_k) &
  !$acc device(nonaka_file)
...
103 format (1x, a3, 2x, a32, 1x, "=", 1x, l)
 ...
 "small_x", small_x
  ...
  ltest = disable_thermal_neutrinos .eqv. .false.
  write (unit,103) merge("   ", "[*]", ltest), &
 "disable_thermal_neutrinos", disable_thermal_neutrinos
 ...
 "ode_max_steps", ode_max_steps
  ...
  ltest = use_jacobian_caching .eqv. .true.
  write (unit,103) merge("   ", "[*]", ltest), &
 "use_jacobian_caching", use_jacobian_caching
 ...
 "nonaka_k", nonaka_k
  ...
  ltest = nonaka_file == "nonaka_plot.dat"
  write (unit,100) merge("   ", "[*]", ltest), &
 "nonaka_file", trim(nonaka_file)
