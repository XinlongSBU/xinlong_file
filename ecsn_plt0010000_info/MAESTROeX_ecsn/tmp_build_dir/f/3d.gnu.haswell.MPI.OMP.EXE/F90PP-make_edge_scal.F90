...
subroutine make_edge_scal_predictor_3d(lo, hi, idir, domlo, domhi, &
  ...
  double precision, intent(in   ) :: vmac  (v_lo(1):v_hi(1),v_lo(2):v_hi(2),v_lo(3):v_hi(3))
  double precision, intent(in   ) :: wmac  (w_lo(1):w_hi(1),w_lo(2):w_hi(2),w_lo(3):w_hi(3))
  double precision, intent(inout) :: Ip(ip_lo(1):ip_hi(1),ip_lo(2):ip_hi(2),ip_lo(3):ip_hi(3),3)
  double precision, intent(inout) :: Im(im_lo(1):im_hi(1),im_lo(2):im_hi(2),im_lo(3):im_hi(3),3)
        ...
        do j=lo(2),hi(2)
           do i=lo(1),hi(1)
              ...
              if (i .eq. domlo(1)) then
                 if (adv_bc(1,1,bccomp) .eq. 3) then
                    sl(i,j,k) = s(i-1,j,k,comp)
                    sr(i,j,k) = s(i-1,j,k,comp)
                 else if (adv_bc(1,1,bccomp) .eq. 2 .or. &
                      adv_bc(1,1,bccomp) .eq. 4) then
                    if (is_vel .eq. 1 .and. comp .eq. 1) then
                       sr(i,j,k) = min(sr(i,j,k),0.d0)
                    end if
                    sl(i,j,k) = sr(i,j,k)
                 else if (adv_bc(1,1,bccomp) .eq. 1) then
                    sl(i,j,k) = sr(i,j,k)
                 else if (adv_bc(1,1,bccomp) .eq. -1) then
                    sl(i,j,k) = 0.d0
                    sr(i,j,k) = 0.d0
                 else if (adv_bc(1,1,bccomp) .eq. 0) then
                 else
              ...
              if (i .eq. domhi(1)+1) then
                 if (adv_bc(1,2,bccomp) .eq. 3) then
                    sl(i,j,k) = s(i,j,k,comp)
                    sr(i,j,k) = s(i,j,k,comp)
                 else if (adv_bc(1,2,bccomp) .eq. 2 .or. &
                      adv_bc(1,2,bccomp) .eq. 4) then
                    if (is_vel .eq. 1 .and. comp .eq. 1) then
                       sl(i,j,k) = max(sl(i,j,k),0.d0)
                    end if
                    sr(i,j,k) = sl(i,j,k)
                 else if (adv_bc(1,2,bccomp) .eq. 1) then
                    sr(i,j,k) = sl(i+1,j,k)
                 else if (adv_bc(1,2,bccomp) .eq. -1) then
                    sl(i,j,k) = 0.d0
                    sr(i,j,k) = 0.d0
                 else if (adv_bc(1,2,bccomp) .eq. 0) then
                 else
        ...
        do j=lo(2),hi(2)
           do i=lo(1),hi(1)
              ...
              if (j .eq. domlo(2)) then
                 if (adv_bc(2,1,bccomp) .eq. 3) then
                    sl(i,j,k) = s(i,j-1,k,comp)
                    sr(i,j,k) = s(i,j-1,k,comp)
                 else if (adv_bc(2,1,bccomp) .eq. 2 .or. &
                      adv_bc(2,1,bccomp) .eq. 4) then
                    if (is_vel .eq. 1 .and. comp .eq. 2) then
                       sr(i,j,k) = min(sr(i,j,k),0.d0)
                    end if
                    sl(i,j,k) = sr(i,j,k)
                 else if (adv_bc(2,1,bccomp) .eq. 1) then
                    sl(i,j,k) = sr(i,j,k)
                 else if (adv_bc(2,1,bccomp) .eq. -1) then
                    sl(i,j,k) = 0.d0
                    sr(i,j,k) = 0.d0
                 else if (adv_bc(2,1,bccomp) .eq. 0) then
                 else
              ...
              if (j .eq. domhi(2)+1) then
                 if (adv_bc(2,2,bccomp) .eq. 3) then
                    sl(i,j,k) = s(i,j,k,comp)
                    sr(i,j,k) = s(i,j,k,comp)
                 else if (adv_bc(2,2,bccomp) .eq. 2 .or. &
                      adv_bc(2,2,bccomp) .eq. 4) then
                    if (is_vel .eq. 1 .and. comp .eq. 2) then
                       sl(i,j,k) = max(sl(i,j,k),0.d0)
                    end if
                    sr(i,j,k) = sl(i,j,k)
                 else if (adv_bc(2,2,bccomp) .eq. 1) then
                    sr(i,j,k) = sl(i,j,k)
                 else if (adv_bc(2,2,bccomp) .eq. -1) then
                    sl(i,j,k) = 0.d0
                    sr(i,j,k) = 0.d0
                 else if (adv_bc(2,2,bccomp) .eq. 0) then
                 else
        ...
        do j=lo(2),hi(2)
           do i=lo(1),hi(1)
              ...
              if (k .eq. domlo(3)) then
                 if (adv_bc(3,1,bccomp) .eq. 3) then
                    sl(i,j,k) = s(i,j,k,comp)
                    sr(i,j,k) = s(i,j,k,comp)
                 else if (adv_bc(3,1,bccomp) .eq. 2 .or. &
                      adv_bc(3,1,bccomp) .eq. 4) then
                    if (is_vel .eq. 1 .and. comp .eq. 3) then
                       sr(i,j,k) = min(sr(i,j,k),0.d0)
                    end if
                    sl(i,j,k) = sr(i,j,k)
                 else if (adv_bc(3,1,bccomp) .eq. 1) then
                    sl(i,j,k) = sr(i,j,k)
                 else if (adv_bc(3,1,bccomp) .eq. -1) then
                    sl(i,j,k) = 0.d0
                    sr(i,j,k) = 0.d0
                 else if (adv_bc(3,1,bccomp) .eq. 0) then
                 else
              ...
              if (k .eq. domhi(3)+1) then
                 if (adv_bc(3,2,bccomp) .eq. 3) then
                    sl(i,j,k) = s(i,j,k,comp)
                    sr(i,j,k) = s(i,j,k,comp)
                 else if (adv_bc(3,2,bccomp) .eq. 2 .or. &
                      adv_bc(3,2,bccomp) .eq. 4) then
                    if (is_vel .eq. 1 .and. comp .eq. 3) then
                       sl(i,j,k) = max(sl(i,j,k),0.d0)
                    end if
                    sr(i,j,k) = sl(i,j,k)
                 else if (adv_bc(3,2,bccomp) .eq. 1) then
                    sr(i,j,k) = sl(i,j,k)
                 else if (adv_bc(3,2,bccomp) .eq. -1) then
                    sl(i,j,k) = 0.d0
                    sr(i,j,k) = 0.d0
                 else if (adv_bc(3,2,bccomp) .eq. 0) then
                 else
