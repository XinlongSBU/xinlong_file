          ...
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                ...
                if (spherical .eq. 1) then
                   ...
                   ! solve Riemann problem using full velocity
                   uavg = HALF*(wlz+wrz)
                   test = ((wlz+w0_cart(i,j,k,3).le.ZERO .and. wrz+w0_cart(i,j,k,3).ge.ZERO) .or. &
                        (abs(wlz+wrz+TWO*w0_cart(i,j,k,3)) .lt. rel_eps))
                   wtrans(i,j,k) = merge(wlz,wrz,uavg+w0_cart(i,j,k,3) .gt. ZERO)
                   wtrans(i,j,k) = merge(ZERO,wtrans(i,j,k),test)
                end if
             enddo
          enddo
