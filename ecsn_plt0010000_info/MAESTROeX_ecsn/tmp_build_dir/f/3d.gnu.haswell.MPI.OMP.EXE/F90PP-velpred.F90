...
subroutine velpred_interface_3d(lo, hi, idir, domlo, domhi, &
  ...
  double precision, intent(in   ) :: Imu (imu_lo(1):imu_hi(1),imu_lo(2):imu_hi(2),imu_lo(3):imu_hi(3),3)
  double precision, intent(in   ) :: Ipu (ipu_lo(1):ipu_hi(1),ipu_lo(2):ipu_hi(2),ipu_lo(3):ipu_hi(3),3)
  double precision, intent(in   ) :: Imv (imv_lo(1):imv_hi(1),imv_lo(2):imv_hi(2),imv_lo(3):imv_hi(3),3)
  double precision, intent(in   ) :: Ipv (ipv_lo(1):ipv_hi(1),ipv_lo(2):ipv_hi(2),ipv_lo(3):ipv_hi(3),3)
  double precision, intent(in   ) :: Imw (imw_lo(1):imw_hi(1),imw_lo(2):imw_hi(2),imw_lo(3):imw_hi(3),3)
  double precision, intent(in   ) :: Ipw (ipw_lo(1):ipw_hi(1),ipw_lo(2):ipw_hi(2),ipw_lo(3):ipw_hi(3),3)
  double precision, intent(inout) :: ul (ul_lo(1):ul_hi(1),ul_lo(2):ul_hi(2),ul_lo(3):ul_hi(3),3)
  double precision, intent(inout) :: ur (ur_lo(1):ur_hi(1),ur_lo(2):ur_hi(2),ur_lo(3):ur_hi(3),3)
  double precision, intent(inout) :: uimh (ui_lo(1):ui_hi(1),ui_lo(2):ui_hi(2),ui_lo(3):ui_hi(3),3)
...
subroutine velpred_transverse_3d(lo, hi, base_dir, norm_dir, &
  ...
  double precision, intent(in   ) :: ulx (ulx_lo(1):ulx_hi(1),ulx_lo(2):ulx_hi(2),ulx_lo(3):ulx_hi(3),3)
  double precision, intent(in   ) :: urx (urx_lo(1):urx_hi(1),urx_lo(2):urx_hi(2),urx_lo(3):urx_hi(3),3)
  double precision, intent(in   ) :: uimhx (uix_lo(1):uix_hi(1),uix_lo(2):uix_hi(2),uix_lo(3):uix_hi(3),3)
  double precision, intent(in   ) :: uly (uly_lo(1):uly_hi(1),uly_lo(2):uly_hi(2),uly_lo(3):uly_hi(3),3)
  double precision, intent(in   ) :: ury (ury_lo(1):ury_hi(1),ury_lo(2):ury_hi(2),ury_lo(3):ury_hi(3),3)
  double precision, intent(in   ) :: uimhy (uiy_lo(1):uiy_hi(1),uiy_lo(2):uiy_hi(2),uiy_lo(3):uiy_hi(3),3)
  double precision, intent(in   ) :: ulz (ulz_lo(1):ulz_hi(1),ulz_lo(2):ulz_hi(2),ulz_lo(3):ulz_hi(3),3)
  double precision, intent(in   ) :: urz (urz_lo(1):urz_hi(1),urz_lo(2):urz_hi(2),urz_lo(3):urz_hi(3),3)
  double precision, intent(in   ) :: uimhz (uiz_lo(1):uiz_hi(1),uiz_lo(2):uiz_hi(2),uiz_lo(3):uiz_hi(3),3)
...
subroutine velpred_3d(lo, hi, lev, idir, domlo, domhi, &
  ...
  double precision, intent(in   ) :: Imf (imf_lo(1):imf_hi(1),imf_lo(2):imf_hi(2),imf_lo(3):imf_hi(3),3)
  double precision, intent(in   ) :: Ipf (ipf_lo(1):ipf_hi(1),ipf_lo(2):ipf_hi(2),ipf_lo(3):ipf_hi(3),3)
  double precision, intent(in   ) :: ul (ul_lo(1):ul_hi(1),ul_lo(2):ul_hi(2),ul_lo(3):ul_hi(3),3)
  double precision, intent(in   ) :: ur (ur_lo(1):ur_hi(1),ur_lo(2):ur_hi(2),ur_lo(3):ur_hi(3),3)
  ...
  double precision, intent(in   ) :: w0_cart(w_lo(1):w_hi(1),w_lo(2):w_hi(2),w_lo(3): w_hi(3),3)
  double precision, intent(in   ) :: dx(3)
  double precision, value, intent(in   ) :: dt
  integer         , intent(in   ) :: phys_bc(3,2) ! dim, lohi, (comp)
        ...
        do j=lo(2),hi(2)
           do i=lo(1),hi(1)
              ...
              if (spherical .eq. 1) then
                 ...
                 ! solve Riemann problem using full velocity
                 uavg = HALF*(wmacl+wmacr)
                 test = ((wmacl+w0_cart(i,j,k,3) .le. ZERO .and. &
                      wmacr+w0_cart(i,j,k,3) .ge. ZERO) .or. &
                      (abs(wmacl+wmacr+TWO*w0_cart(i,j,k,3)) .lt. rel_eps))
                 wmac(i,j,k) = merge(wmacl,wmacr,uavg+w0_cart(i,j,k,3) .gt. ZERO)
                 wmac(i,j,k) = merge(ZERO,wmac(i,j,k),test)
