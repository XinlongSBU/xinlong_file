...
contains
  ...
  subroutine make_vorticity_2d(lo,hi,vel,v_lo,v_hi,dx,vort,d_lo,d_hi,bc)
    ...
    if (bc(1,1) .eq. 1 .or. bc(1,1) .eq. 4 .or. bc(1,1) .eq. 5) then
       i = lo(1)
       do j = lo(2), hi(2)
    ...
    if (bc(1,2) .eq. 1 .or. bc(1,2) .eq. 4 .or. bc(1,2) .eq. 5) then
       i = hi(1)
       do j = lo(2), hi(2)
          vx = -(vel(i-1,j,k,2) + 3.d0*vel(i,j,k,2) - 4.d0*vel(i+1,j,k,2)) / dx(1)
          uy = (vel(i,j+1,k,1) - vel(i,j-1,k,1)) / (2.d0*dx(2))
    ...
    if (bc(2,1) .eq. 1 .or. bc(2,1) .eq. 4 .or. bc(2,1) .eq. 5) then
       j = lo(2)
       do i = lo(1), hi(1)
          vx = (vel(i+1,j,k,2) - vel(i-1,j,k,2)) / (2.d0*dx(1))
    ...
    if (bc(2,2) .eq. 1 .or. bc(2,2) .eq. 4 .or. bc(2,2) .eq. 5) then
       j = hi(2)
       do i = lo(1), hi(1)
          vx =  (vel(i+1,j,k,2) - vel(i-1,j,k,2)) / (2.d0*dx(1))
          uy = -(vel(i,j-1,k,1) + 3.d0*vel(i,j,k,1) - 4.d0*vel(i,j+1,k,1)) / dx(2)
  ...
  subroutine make_vorticity_3d(lo,hi,vel,v_lo,v_hi,dx,vort,d_lo,d_hi,bc)
    ...
    fix_lo_x = ( bc(1,1) .eq. 1 .or. bc(1,1) .eq. 5 )
    fix_hi_x = ( bc(1,2) .eq. 1 .or. bc(1,2) .eq. 5 )

    fix_lo_y = ( bc(2,1) .eq. 1 .or. bc(2,1) .eq. 5 )
    fix_hi_y = ( bc(2,2) .eq. 1 .or. bc(2,2) .eq. 5 )

    fix_lo_z = ( bc(3,1) .eq. 1 .or. bc(3,1) .eq. 5 )
    fix_hi_z = ( bc(3,2) .eq. 1 .or. bc(3,2) .eq. 5 )
       ...
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             magvel(i,j,k) = sqrt(  vel(i,j,k,1)**2 + &
                  vel(i,j,k,2)**2 + &
                  ( vel(i,j,k,3) + 0.5d0*(w0_cart(i,j,k,3) + w0_cart(i,j,k+1,3)) )**2 )
          enddo
       enddo
    enddo
  ...
  subroutine make_divw0(lo,hi,w0_cart,w_lo,w_hi,nc_w0,dx,divw0,d_lo,d_hi) bind(C,name="make_divw0")
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             divw0(i,j,k) = (w0_cart(i,j,k+1,3) - w0_cart(i,j,k,3)) / dx(3)
          enddo
       enddo
    enddo
