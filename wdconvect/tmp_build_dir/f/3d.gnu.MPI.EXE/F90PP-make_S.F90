       ...
       do j = lo(2),hi(2)
          do i = lo(1),hi(1)
             ...
             if (use_delta_gamma1_term .and. r < anelastic_cutoff_coord(lev)) then
                if (r .eq. 0) then
                   gradp0 = (p0(lev,r+1) - p0(lev,r))/dx(3)
                else if (r .eq. nr(lev)-1) then
                   gradp0 = (p0(lev,r) - p0(lev,r-1))/dx(3)
                else
                   gradp0 = 0.5d0*(p0(lev,r+1) - p0(lev,r-1))/dx(3)
                endif

                delta_gamma1(i,j,k) = eos_state%gam1 - gamma1bar(lev,r)

                delta_gamma1_term(i,j,k) = (eos_state%gam1 - gamma1bar(lev,r))*u(i,j,k,3)* &
